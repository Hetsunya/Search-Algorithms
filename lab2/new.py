...
def main():
    ...
    # Дополнительный тест для равновзвешенного графа
    # print("\nТест для равновзвешенного графа (все веса = 1):")
    equal_graph = generate_graph(10, equal_weight=True)
    # bfs_eq_order = bfs(equal_graph, 0)
    # _, dijkstra_eq_order = dijkstra(equal_graph, 0)
    # print("Порядок обхода BFS:", bfs_eq_order, "Длина обхода: ", len(bfs_eq_order))
    # print("Порядок обхода Дейкстры:", dijkstra_eq_order, "Длина обхода: ", len(dijkstra_eq_order))
    # visualize_graph(equal_graph, title="Равновзвешенный граф (все веса = 1)")

    # Дополнительный тест с целевой вершиной для BFS и Дейкстры
    print("\nТест с целевой вершиной 5:")
    print("Граф: ", equal_graph)
    target_vertex = 7
    bfs_target_order = bfs( equal_graph, 0, target=target_vertex)
    dijkstra_target_distances, dijkstra_target_order = dijkstra(equal_graph, 0, target=target_vertex)
